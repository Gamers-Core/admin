...
import { FullUser, BackendError, SearchUsersSchema, gamersCoreAdmin } from '@/api';

const queryKey = (searchOptions: SearchUsersSchema = {}) =>
  ['admin-users', ...Object.entries(searchOptions).sort(([a], [b]) => a.localeCompare(b))] as const;

type QueryKey = ReturnType<typeof queryKey>;

const queryFn = ({ queryKey: [, ...paramsArr] }: QueryFunctionContext<QueryKey>) =>
  gamersCoreAdmin
    .get<FullUser[], AxiosResponse<FullUser[]>>('/users', { params: Object.fromEntries(paramsArr) })
    .then((res) => res.data);
...
export const useInvalidateAdminUsersQuery = (searchOptions: SearchUsersSchema = {}) => {
